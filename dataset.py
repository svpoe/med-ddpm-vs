...
class NiftiPairImageGenerator(Dataset):
    ...
    #     for input_file, target_file in zip(input_files, target_files):
    #         # Extract a consistent ID from filenames (e.g., digits at the end)
    #         input_id = re.findall(r'\d+', os.path.basename(input_file))
    #         target_id = re.findall(r'\d+', os.path.basename(target_file))

    #         # Sanity check
    #         print('input_id: ', input_id)
    #         print('target_id: ', target_id)

    #         if not input_id or not target_id or input_id[0] != target_id[0]:
    #             print(f"❌ Mismatch: {os.path.basename(input_file)} vs {os.path.basename(target_file)}")
    #             continue
